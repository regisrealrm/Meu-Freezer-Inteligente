...
export default function App() {
  ...
  useEffect(()=>{
    try {
      ...
      const localData = localStorage.getItem("mfi_local_data");
      if(localData) {
        const d = JSON.parse(localData);
        if(d.meats?.length||d.exits?.length||d.catalog?.length) {
          const loadedMeats = d.meats || [];
          setMeats(loadedMeats);
          setExits(d.exits    || []);
          setCatalog(d.catalog || []);

          // Auto-heal: restaura qualquer valor que ainda existe nas carnes
          // mas foi deletado do config acidentalmente
          const loadedConfig = d.appConfig || {
            tipos:[...TIPOS], locais:[...LOCAIS], origens:[...ORIGENS], utilidades:["churrasco","consumo"]
          };
          const heal = (arr, field) => {
            const inUse = [...new Set(loadedMeats.map(m=>m[field]).filter(Boolean))];
            const missing = inUse.filter(v=>!arr.includes(v));
            return missing.length ? [...arr, ...missing] : arr;
          };
          const healed = {
            ...loadedConfig,
            tipos:      heal(loadedConfig.tipos      || [...TIPOS],      "tipo"),
            locais:     heal(loadedConfig.locais     || [...LOCAIS],     "local"),
            origens:    heal(loadedConfig.origens    || [...ORIGENS],    "origem"),
            utilidades: heal(loadedConfig.utilidades || ["churrasco","consumo"], "utilidade"),
          };
          setAppConfig(healed);
          localTs.current   = d._ts || 0;
          lastSaved.current = localData;
        }
      }
    } catch(e){}
    setLoaded(true);
    ...
    try {
      const unsubscribe = onValue(dbRef(db, DB_PATH), (snapshot)=>{
        ...
        const fbMeats  = data.meats || [];
        const fbConfig = data.appConfig || {};
        const heal = (arr,field) => {
          const inUse = [...new Set(fbMeats.map(m=>m[field]).filter(Boolean))];
          const miss  = inUse.filter(v=>!arr.includes(v));
          return miss.length ? [...arr,...miss] : arr;
        };
        const healed = {
          tipos:      heal(fbConfig.tipos      ||[...TIPOS],      "tipo"),
          locais:     heal(fbConfig.locais     ||[...LOCAIS],     "local"),
          origens:    heal(fbConfig.origens    ||[...ORIGENS],    "origem"),
          utilidades: heal(fbConfig.utilidades ||["churrasco","consumo"],"utilidade"),
        };
